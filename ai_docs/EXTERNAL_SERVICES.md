...
**This whole layout is being replaced.** Measured `time_total` from Kazakhstan: `europe-west1` 0.433s, `europe-central2` 0.451s, `us-central1` 0.582s, `asia-south1` **1.193s** — the database currently sits in the slowest measured region for its users. Migration of Firestore, Storage and all functions to a single `europe-west1` project is planned in `ai_specs/047-migrate-firebase-region-europe-west1-plan.md`, with the name-independent preparation split out into `ai_specs/046-prepare-region-migration-plan.md`. Update this section when that lands.
