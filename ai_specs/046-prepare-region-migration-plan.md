...
- [ ] **НЕ СДЕЛАНО — ручная проверка отложена пользователем (2026-08-15).** Новые правила уже живут в проде, но глазами их никто не видел. Проверить в бизнес-приложении:
  - [ ] загрузка обычной картинки товара (JPEG/PNG до 5 MB) проходит — заодно подтверждает фикс `isAdmin()`
  - [ ] **удаление картинки работает** — самое важное: именно здесь формулировка из плана дала бы регресс. Создать товар и отменить так, чтобы сработала компенсация в `create_item_form_controller.dart:69`, либо заменить картинку у существующего товара
  - [ ] файл больше 5 MB отклоняется
  - [ ] не-изображение отклоняется (можно пропустить, если UI фильтрует по типу — правило покрыто тестом)
